...
register_page(
    __name__,
    name='Average Trading Price Predictions :',
    top_nav=True,
    path='/average_trading_price.py'
)
...
layout = html.Div([
    ...
    dbc.Row([
        dcc.Graph(id='tr_avg_pred')
    ])
    ], style={'padding': '20px'})

@callback(
    Output(component_id='tr_avg_pred', component_property='figure', allow_duplicate=True),
...
def update_graph(collection_value, n_clicks):
    ...
    if n_clicks is None:
        ...
        fig = mdl.class_launcher(collection_value,1, 24, True, 'average_trading_price')
    return fig
